@tool
extends Marker3D
class_name TentSpawnPoint

@export var tent_scene: PackedScene : set = _set_tent_scene
...
func _set_tent_scene(new_scene: PackedScene):
	tent_scene = new_scene
	if Engine.is_editor_hint():
		_update_preview()
...
func _update_preview():
	...
	if tent_scene:
		var preview = tent_scene.instantiate()
