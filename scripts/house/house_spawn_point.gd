@tool
extends Marker3D
class_name HouseSpawnPoint

@export var house_scene: PackedScene : set = _set_house_scene
...
func _set_house_scene(new_scene: PackedScene):
	house_scene = new_scene
	if Engine.is_editor_hint():
		_update_preview()
...
func _update_preview():
	...
	if house_scene:
		var preview = house_scene.instantiate()
