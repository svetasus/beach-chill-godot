...
func get_save_path(player_id: int) -> String:
	...
	return "user://tent_data_" + account_id + ".save"
...
func save_tent_for_player(player_id: int, is_disconnecting: bool):
	...
	if items_container:
		...
		for i in range(children.size() - 1, -1, -1):
			...
			query.position = item.global_position
	...
	if json.has("items"):
		...
		if items_container:
			for item_data in json["items"]:
				...
				new_item.name = "LoadedItem_" + str(Time.get_ticks_msec()) + "_" + str(randi() % 1000)
				...
				new_item.global_rotation = rot

				# Yielding to allow item to enter tree and set data path safely
				await get_tree().process_frame
